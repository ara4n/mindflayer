# taken from https://www.data-to-viz.com/story/MapConnection.html
...
data <- read.table(paste("conns/conns-", day, ".tsv", sep=''), header=T, sep='\t')
...
p <- ggplot() +
  ...
  scale_color_distiller(palette="YlOrRd", limits=c(0, 40000)) + # Options are YlOrRd or PuBuGn 
  ...
  ggplot2::annotate("text", x = -170, y = -51, hjust = 0, size = 8, label = paste("Top 100 homeservers by DAU", day), color = "white", alpha = 0.5) +
  ylim(-60,80) +
  scale_x_continuous(expand = c(0.006, 0.006), lim = c(-180, 180)) +
  coord_equal()

# Save as PNG
ggsave(paste("IMG/matrix-", day, ".png", sep=''), width = 36, height = 15.22, units = "in", dpi = 90)
